...
import type { ChartsByCountryResponse } from "./types/get-charts-by-country-response";
...
type GetChartsByCountryCodeArgs = {
...
const shazamCoreApi = createApi({
  ...
  baseQuery: fetchBaseQuery({
    ...
    prepareHeaders: (headers) => {
      headers.set("x-rapidapi-key", import.meta.env.VITE_RAPIDAPI_API_KEY);
  ...
  endpoints: (builder) => ({
    ...
    getTrackDetailsById: builder.query<
    ...
    getArtistDetailsById: builder.query<
      ...
      GetArtistDetailsArgs
    >({
      query: ({ artistId }) => `v2/artists/details?artist_id=${artistId}`,
    }),
    getChartsByCountryCode: builder.query<
      ChartsByCountryResponse[],
      GetChartsByCountryCodeArgs
    >({
      query: ({ countryCode }) =>
...
export const {
  useGetTopCharsQuery,
  useGetTrackDetailsByIdQuery,
  useGetArtistDetailsByIdQuery,
  useGetChartsByCountryCodeQuery,
} = shazamCoreApi;
